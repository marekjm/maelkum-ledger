...
def legacy_output_table(expenses, revenues, column, include_percentage=False):
    ...
    if len(expenses) > 1:
        ...
        mean_expense = util.math.mean(
            list(map(lambda each: abs(each["value"]["amount"]), expenses))
        )
        median_expense = util.math.median(
            list(map(lambda each: abs(each["value"]["amount"]), expenses))
        )
        p(
            "  Expenses range: {:.2f} - {:.2f} {}".format(
                min_expense, max_expense, DEFAULT_CURRENCY
            )
        )
    ...
    if include_percentage:
        ...
        if percentage_spent > DEFICIT_THRESHOLD:
            p(
                "  Deficit is {:.4f}% of revenues, or {:.2f} {}.".format(
            ...
            p(
                "  Revenues covered about {:.4f}% of expenses.".format(
    ...
    for each in book["transactions"]:
        ...
        if each["type"] == "expense":
            ...
            if value["currency"] != DEFAULT_CURRENCY:
                c = value["currency"]
                r = book["currency_basket"][(c, DEFAULT_CURRENCY)]["rates"]["buy"][
                    "weighted"
                ]
...
def legacy_report_period_impl(
    book, period_begin, period_end, period_name, column, table_opts={}
):
    expenses = []
    revenues = []
    for each in book["transactions"]:
    ...
    Book.screen.print(
        ...
        text="{} ({} to {})".format(
            ...
            colorise_if_possible(
                COLOR_DATETIME, period_begin.strftime(DAYSTAMP_FORMAT)
            ),
...
def legacy_report_prev_7_days(book, column):
    today = datetime.datetime.strptime(
        (datetime.datetime.now() - datetime.timedelta(days=8)).strftime(
            TIMESTAMP_ZERO_FORMAT
        ),
        TIMESTAMP_FORMAT,
    )
    last_week = datetime.datetime.strptime(
        (datetime.datetime.now() - datetime.timedelta(days=14)).strftime(
            TIMESTAMP_ZERO_FORMAT
        ),
        TIMESTAMP_FORMAT,
    )
...
def legacy_report_a_month_impl(book, first_day, last_day, name, column):
    expenses, revenues = Book.report_period_impl(
        book,
        first_day,
        last_day,
        name,
    ...
    total_expenses = abs(
        sum(
            map(
                lambda each: each["value"]["amount"],
                filter(
                    lambda each: each["value"]["currency"] == DEFAULT_CURRENCY, expenses
                ),
            )
        )
    )
    ...
    Book.screen.print(
        column=column,
        text="  Average daily expense is {:.2f} {}.".format(
            daily_average,
            DEFAULT_CURRENCY,
        ),
    )
    ...
    for each in expenses:
        ...
        if each["value"]["currency"] != DEFAULT_CURRENCY:
            continue
        daily_median[each["timestamp"].strftime(DAYSTAMP_FORMAT)] += each["value"][
            "amount"
        ]
    daily_median = util.math.median(sorted(map(abs, daily_median.values())))
    Book.screen.print(
        column=column,
        text="  Median daily expense is {:.2f} {}.".format(
            daily_median,
            DEFAULT_CURRENCY,
        ),
    )
...
def legacy_report_this_month(book, column):
    ...
    expenses, revenues = Book.report_a_month_impl(
        book, this_month, today, "This month", column
    )
    total_expenses = sum(
        map(
            lambda each: each["value"]["amount"],
    ...
    total_revenues = sum(
        map(
            lambda each: each["value"]["amount"],
            filter(
                lambda each: each["value"]["currency"] == DEFAULT_CURRENCY, revenues
            ),
        )
    )
    ...
    if spending_target is not None:
        ...
        if spending_target[0] == "%":
            ...
            available_budgeted_funds = (total_revenues * target_percentage) - abs(
                total_expenses
            )
        elif spending_target[0] == "$":
            available_budgeted_funds = spending_target[1] - total_expenses
        available_budgeted_funds_per_day = (
            available_budgeted_funds / days_left_in_this_month
        )
        fmt = "{:7.2f}"
        Book.screen.print(
            column=column,
            text="  Daily expense cap to meet budget: {} {}".format(
                colorise_if_possible(
...
def legacy_first_recorded_transaction(book):
    return sorted(book["transactions"], key=lambda each: each["timestamp"])[0]
