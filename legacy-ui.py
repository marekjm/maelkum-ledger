    ...
    def parse(lines, convert=decimal.Decimal):
        ...
        for each in groups:
            ...
            if head[0] == KEYWORD_OPEN:
                ...
                if not balance:
                    raise Exception(
                        "{} account {} does not have an opening balance".format(
                            account_type,
                            account_name,
                        )
                    )
            ...
            if head[0] == KEYWORD_CURRENCY_RATES:
                ...
                for each in map(lambda e: e[0], body):
                    ...
                    if ledger.book.DEFAULT_CURRENCY not in currency_pair:
                        raise Exception(
                            "invalid currency pair: {}".format("/".join(currency_pair))
                        )
            ...
            if head[0] in (
                ...
                if tx_kind == TX_EXPENSE_T:
                    ...
                    if not source_account:
                        raise Exception(
                            "{}: no source account found for expense".format(
                                head[1],
                            )
                        )
                    ...
                    if not dest_account:
                        raise Exception(
                            "{}: no destination account found for expense".format(
                                head[1],
                            )
                        )
                    ...
                    for pat in patterns:
                        if pat["what"] != tx_kind:
                            continue
                        if (source_account in pat["accounts"]) or (
                            dest_account in pat["accounts"]
                        ):
                    ...
                    if not source_account:
                        raise Exception(
                            "{}: no source account found for revenue".format(
                                head[1],
                            )
                        )
                    source_account = source_account[0][1]
                    ...
                    if not dest_account:
                        raise Exception(
                            "{}: no destination account found for revenue".format(
                                head[1],
                            )
                        )
                    dest_account = dest_account[0][1]
                    ...
                    for x in accounts:
                        if x[0] != OWN_ACCOUNT_T:
                            raise Exception(
                                "{}: invalid account for transfer (not owned): {}".format(
                                    head[1],
                                    x[1],
                                )
                            )
                    ...
                    try:
                        ...
                        raise Exception(
                            "{}: no currency on transfer from {}".format(
                                head[1],
                                source_account[0],
                            )
                        )
                    ...
                    any_fee = list(
                        map(
                            lambda s: s[0].split()[1:],
                            filter(lambda s: s[0].startswith("fee: "), extra),
                        )
                    )
                    fee, fee_currency = first_or(
                        any_fee,
                        (
                            "0.00",
                            source_currency,
                    ...
                    if source_currency == dest_currency and not balanced_amounts:
                        raise Exception(
                            "amounts are not balanced: {} {} != {} {}".format(
                                source_account[1],
                                source_currency,
                                dest_account[1],
                                dest_currency,
                    ...
                    is_equity_tx = is_equity_account(dest_account) or is_equity_account(
                        source_account
                    )
                    if is_equity_tx:
                        any_shares = first_or(
                        ...
                        if not any_shares:
                            fmt = (
                                "equity transfer between {} and {} does not"
                    ...
                    if fee:
                        any_intermediary = first_or(
                            list(
                                map(
                                    lambda s: s.split(maxsplit=1)[1],
                                    filter(
                                        lambda s: s.startswith("intermediary: "),
                                        map(first, extra),
                                    ),
                                )
                            )
                        ...
                        if not any_intermediary:
                            fmt = (
                                "share transaction on {} does not specify an"
                                + " intermediary"
                            )
                            raise Exception(
                                fmt.format(
                    ...
                    if source_currency != dest_currency:
                        rate = list(
                            map(
                                lambda x: x[0],
                                filter(lambda x: x[0].startswith("rate:"), extra),
                            )
                        )
                        rate = rate[0].split() if rate else None
                        if rate is None:
                            raise Exception(
                                "{}: no rate on transfer from {} to {}".format(
                                    head[1],
                                    source_account,
                                    dest_account,
                                )
                        ...
                        try:
                            ...
                            raise Exception(
                                "{}: invalid rate on transfer from {} to {}".format(
                                    head[1],
                                    source_account,
                                    dest_account,
                                )
                        ...
                        src_value_converted = op(
                            -1 * value["src"]["amount"], rate_value
                        )
                    ...
                    is_equity = lambda a: (
                        a[1][0].startswith(ledger.book.ACCOUNT_EQUITY_T)
                    )
                    is_asset = lambda a: (
                        a[1][0].startswith(ledger.book.ACCOUNT_ASSET_T)
                    )
                    ...
                    if not source_account:
                        raise Exception(
                            "{}: no source account found for dividend".format(
                                head[1],
                            )
                        )
                    ...
                    if not dest_account:
                        raise Exception(
                            "{}: no destination account found for dividend".format(
                                head[1],
                            )
                        )
                    dest_account = dest_account[0][1]
                    ...
                    book_contents["transactions"][-1][
                        "timestamp"
                    ] = Parser.parse_timestamp(head[1])
                    book_contents["transactions"].append(
                        {
                            "type": "revenue",
                    ...
                    if source_company not in equity_account["companies"]:
                        raise Exception(
                            "{}: company {} does not exist in account: {}".format(
                                head[1],
                                source_company,
                                source_account,
                            )
                ...
                book_contents["transactions"][-1]["timestamp"] = Parser.parse_timestamp(
                    head[1]
                )
        ...
        for k, v in configuration.items():
            if k == "budget":
                ...
                book_contents["budgets"] = {
                    "_target": (
                        kind,
                        decimal.Decimal(limit),
                    ),
                }

        book_contents["transactions"] = sorted(
            book_contents["transactions"],
